...
export class ClienteService {
  ...
  async findOne(id: number) {
    return this.prisma.cliente.findUnique({
      where: { id },
      include: { favoritos: true, pedidos: true },
    });
  }
  ...
  async findFavoritados(id: number) {
    ...
    registros.forEach(
      (
        registro, //pega o ID de cada produto associado ao cliente e coloca numa lista
  ...
  async findPedidos(id: number) {
    ...
    if (pedidos.length === 0) {
      throw new HttpException(
        'esse cliente nao tem pedidos',
        HttpStatus.NOT_FOUND,
        {
          cause: new Error('cliente sem pedidos'),
        },
      );
    }
  }
